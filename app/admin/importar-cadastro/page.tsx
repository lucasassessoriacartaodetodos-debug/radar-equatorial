...
import { parseCadastroExcel, CadastroRow } from "@/lib/import/parser-cadastro";
import { upsertCadastro } from "@/lib/actions/import-cadastro";
...
export default function ImportarCadastroPage() {
  const [validRows, setValidRows] = useState<CadastroRow[]>([]);
  const [invalidRows, setInvalidRows] = useState<{ row: number; errors: string[] }[]>([]);
  const [fileName, setFileName] = useState("");
  const [importing, setImporting] = useState(false);
  const [result, setResult] = useState<{ atualizadas: number; criadas: number; total: number } | null>(null);

  const handleFileSelect = async (file: File) => {
    setFileName(file.name);
    setResult(null);
    try {
      const parsed = await parseCadastroExcel(file);
      setValidRows(parsed.valid);
      setInvalidRows(parsed.invalid);
      if (parsed.valid.length > 0) {
        toast.success(`${parsed.valid.length} franquias válidas encontradas`);
      }
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const res = await upsertCadastro(validRows, fileName, "admin");
      if (res.success) {
        setResult({ atualizadas: res.atualizadas ?? 0, criadas: res.criadas ?? 0, total: res.total ?? 0 });
        toast.success(`Importação concluída: ${res.atualizadas} atualizadas, ${res.criadas} criadas`);
        setValidRows([]);
        setInvalidRows([]);
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h2 className="text-xl font-bold text-graphite-100">Importar Cadastro de Franquias</h2>
        <p className="text-sm text-graphite-400 mt-1">
          Selecione a planilha com os dados de cadastro (estado, grupo, franquia, meta).
        </p>
      </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-graphite-400">
                  {result.atualizadas} franquias atualizadas, {result.criadas} criadas. Total: {result.total}.
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-5">
            ...
            <div className="space-y-1 max-h-40 overflow-y-auto">
              ...
              {invalidRows.length > 20 && (
                <p className="text-xs text-graphite-500">... e mais {invalidRows.length - 20} erros</p>
              )}
            </div>
          </CardContent>
        </Card>
                ...
                <tbody>
                  {validRows.slice(0, 10).map((row, i) => (
                    <tr key={i} className="border-b border-graphite-800">
                      <td className="py-1.5 px-2 text-graphite-300">{row.estado}</td>
                      <td className="py-1.5 px-2 text-graphite-300">{row.grupo}</td>
                      <td className="py-1.5 px-2 text-graphite-100">{row.franquia}</td>
                      <td className="py-1.5 px-2 text-right text-graphite-200">{row.meta_mes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
